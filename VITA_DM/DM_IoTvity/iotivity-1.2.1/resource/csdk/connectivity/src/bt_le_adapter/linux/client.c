...
uint8_t * global_pointer=NULL;
uint8_t global_length =NULL;
uint8_t global_data = NULL;
#if 0
void DATA_WRITE(gpointer data,gpointer user_data)
{
    printf("***********%s\n\n",__func__);
    char *address=NULL;
    GDBusProxy * const device = G_DBUS_PROXY(data);
    uint8_t data_to_write = user_data;
    address =  g_variant_print(g_dbus_proxy_get_cached_property(device,"Address"),TRUE);
    printf("ADDRESS **********    %s\n\n",address);
    for(int i=0;i<count_ble;i++)
    {
        ...
        if(strcmp(address,ble_pointer[i].address)==0)
        {
           ...
           VITA_WRITE_REQUEST(ble_pointer[i].fd,global_data,global_length);
        }
    }
                                           ...
                                           CALEContext * context)
{
    ...
    global_data=data;
    g_list_foreach(context->devices,DATA_WRITE,NULL);*/
#if 0
    for(;Devices->data;)
    {
        characteristic = (GDBusProxy *)Devices->data;
        address =  g_variant_print(g_dbus_proxy_get_cached_property(characteristic,"Address"),TRUE);
        address[18]='\0';
        printf("ADDDRESSSSS %s\n",address);
        for(int i=0;i<count_ble;i++)
        {
            if(strcmp(address,ble_pointer[i].address)==0)
            {

               VITA_WRITE_REQUEST(ble_pointer[i].fd,data,length);
            }
        }
